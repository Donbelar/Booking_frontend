...
<?php
$sql = mysql_query("SELECT * FROM site_pages ORDER BY id");
while($row = mysql_fetch_object($sql)) 
{
	if($row->id==1){
		// Skip the home page as this has its own separate page
		continue;
	}else{
		echo "<tr>";
			echo "<td>$row->pageTitle</td>";
			echo "<td>$row->pageTemplate</td>";
			echo "<td><a href=\"".DIRADMIN."editpage.php?id=$row->id\">Edit</a> | <a href=\"javascript:delpage('$row->id','$row->pageTitle');\">Delete</a></td>";
		echo "</tr>";
	}
}
?>
